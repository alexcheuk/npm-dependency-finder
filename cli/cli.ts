...
interface ParsedInput {
  parent: string; // e.g. axios@1
  child: string; // e.g. form-data@4
  removed: boolean; // --removed flag
  noColor: boolean; // --no-color flag / NO_COLOR env
}
...
function parseArg(spec?: string): { name: string; version: string } | null {
  if (!spec) return null;
  const at = spec.lastIndexOf("@");
  if (at <= 0) return { name: spec, version: "" }; // no version provided
...
async function run() {
  ...
  const prelimNoColor =
    rest.includes("--no-color") || Boolean(process.env.NO_COLOR);
  ...
  if (result.success) {
    ...
    if (result.version) {
      spacer();
      console.log(
        `${cy}${b}Earliest parent version:${r} ${b}${parent.name}@${result.version}${r}`
      );
    }
    if (result.details?.length) {
      spacer();
      console.log(
        `${cy}${b}Dependency occurrences (${result.details.length}):${r}`
      );
      for (const line of result.details) console.log(`${g}  ${line}${r}`);
    }
    spacer();
    process.exit(0);
  } else {
    ...
    if (result.details?.length) {
      ...
      for (const line of result.details) console.error(`${g}  ${line}${r}`);
    }
    spacer();
    process.exit(2);
  }
