...
export async function findCompatibleVersion(params, options = {}) {
    ...
    try {
        ...
        log(`Found ${filteredVersions.length} versions >= ${parentMinVersion || "any"}`);
        ...
        log(`Prioritizing: ${stableVersions.length} stable versions, ${preReleaseVersions.length} pre-release versions`);
        ...
        for (const group of versionGroups) {
            ...
            for (const version of group.versions) {
                ...
                const dependencyResult = await analyzePackageDependencies(`${parentPackage}@${version}`, childPackage, childMinVersion, packageRemoved, { silent: options.silent, logger: options.logger });
...
async function analyzePackageDependencies(parentPackageVersion, childPackage, childMinVersion, packageRemoved, options = {}) {
    ...
    if (!options.silent) {
        ...
        logger(`Found ${dependencyLines.length} instances of '${childPackage}' in the dependency tree:`);
...
const versionResolutionCache = new Map(); // key: name@range
...
async function resolveVersion(name, range) {
    ...
    if (versionResolutionCache.has(key))
        return versionResolutionCache.get(key);
    try {
